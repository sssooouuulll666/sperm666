...
def main(page: Page):
    ...
    def color_change(e):
        ...
        _color = [MY_COLOR, 'purple']
        global color
        color = choice(_color)
        banner.controls = [Text(spans=[TextSpan('PIDOR1', TextStyle(size=95, foreground=Paint(color=color, stroke_width=9, stroke_join='round', style='stroke')))], font_family='Consolas'), Text(spans=[TextSpan('PIDOR1', TextStyle(size=95, color=color))], font_family='Consolas')]
    ...
    def start_attack():
        ...
        attack_window = AlertDialog(modal=True, title=Text('Говно летит', color=color, size=30, text_align=TextAlign.CENTER), content=ProgressBar(width=325, color=color), actions=[TextButton('Закрыть', width=90, height=40, on_click=button_cancel, style=ButtonStyle(color=color))], actions_alignment='end', open=True)
    ...
    def confirmation():
        ...
        confirmation_window = AlertDialog(modal=True, title=Text('ало', color=color, size=30, text_align='center'), content=Text('После запуска атаки и мгновенной\nеё отмены, процесс запуска будет необратим, бла бла бла бла\nкурбатов сын шлюхи!\n\nкинуть говно?'), actions=[TextButton('ПИЗДА', on_click=button_cancel, style=ButtonStyle(color=color)), TextButton('ДА', on_click=button_continue, style=ButtonStyle(color=color))], actions_alignment='end', open=True)
    ...
    # Баннер
    banner = Stack([Text(spans=[TextSpan('PIDOR1', TextStyle(size=95, foreground=Paint(color=color, stroke_width=9, stroke_join='round', style='stroke')))], font_family='Consolas'), Text(spans=[TextSpan('PIDOR1', TextStyle(size=95, color=color))], font_family='Consolas')])
    ...
    # Запуск атаки
    attack_button = ElevatedButton(content=Text('кинуть говно в сапог', size=25), on_click=checking_values, width=190, height=60, color=color, url=URL_CHANNEL, autofocus=True)
